 ...
 * 
 * **编排器 (Orchestrators):**
 * - Sisyphus (西西弗斯): 主编排器，SF Bay Area 工程师风格，负责任务分解和协调
 * - Atlas (阿特拉斯): 主控编排器，持有待办列表，负责高层次任务管理
 * 
 * **专家代理 (Specialists):**
 * - Oracle (神谕者): 战略顾问（GPT-5.2），用于复杂调试和架构决策
 * - Librarian (图书管理员): 多仓库研究专家，用于搜索官方文档、GitHub 代码和外部资源
 * - Explore (探索者): 快速代码库探索（类似 contextual grep），快速定位代码模式
 * - Multimodal Looker: 多模态分析器（Gemini 3 Flash），用于 PDF/图片分析
 * 
 * **咨询顾问 (Advisors):**
 * - Prometheus (普罗米修斯): 规划器，负责任务分析和计划制定（未在此导出）
 * - Metis (墨提斯): 前置规划分析，检测计划中的缺陷和盲点
 * - Momus (摩穆斯): 计划审查者，以严格标准审查工作计划
 * 
 * **动态提示词系统:**
...
// 动态提示词构建器的类型：用于在 Sisyphus/Atlas 提示词中展示可用资源
...
// === 专家代理导出 ===
// Oracle (神谕者): 战略顾问，只读模式，用于高质量推理
export { createOracleAgent, ORACLE_PROMPT_METADATA } from "./oracle"

// Librarian (图书管理员): 外部资源搜索（官方文档、开源实现、GitHub 代码）
export { createLibrarianAgent, LIBRARIAN_PROMPT_METADATA } from "./librarian"

// Explore (探索者): 内部代码库快速搜索（contextual grep）
