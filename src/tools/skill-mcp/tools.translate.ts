...
function validateOperationParams(args: SkillMcpArgs): OperationType {
  ...
  if (operations.length === 0) {
    throw new Error(
      `缺少操作。必须恰好指定 tool_name、resource_name 或 prompt_name 中的一个。\n\n` +
        `示例:\n` +
  ...
  if (operations.length > 1) {
    ...
    throw new Error(
      `指定了多个操作。必须恰好提供 tool_name、resource_name 或 prompt_name 中的一个。\n\n` +
        `收到: ${provided}\n\n` +
        `每个操作请使用单独的调用。`,
    )
  }
  ...
  for (const skill of skills) {
    if (skill.mcpConfig) {
      for (const serverName of Object.keys(skill.mcpConfig)) {
        mcps.push(`  - "${serverName}" 来自技能 "${skill.name}"`)
      }
    }
  }
...
function parseArguments(argsJson: string | Record<string, unknown> | undefined): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error(
      `无效的参数 JSON: ${errorMessage}\n\n` +
        `期望一个有效的 JSON 对象，例如: '{"key": "value"}'\n` +
        `收到: ${argsJson}`,
    )
  }
}
...
export function createSkillMcpTool(options: SkillMcpToolOptions): ToolDefinition {
  ...
  return tool({
    ...
    args: {
      mcp_name: tool.schema.string().describe("技能配置中的 MCP 服务器名称"),
      ...
      prompt_name: tool.schema.string().optional().describe("要获取的 MCP 提示词"),
      arguments: tool.schema
        .union([tool.schema.string(), tool.schema.record(tool.schema.string(), tool.schema.unknown())])
        .optional()
        .describe("JSON 字符串或参数对象"),
      grep: tool.schema
        .string()
        .optional()
        .describe("用于过滤输出行的正则表达式模式（仅返回匹配的行）"),
    },
    async execute(args: SkillMcpArgs) {
      ...
      if (!found) {
        throw new Error(
          `未找到 MCP 服务器 "${args.mcp_name}"。\n\n` +
            `已加载技能中的可用 MCP 服务器:\n` +
            formatAvailableMcps(skills) +
            `\n\n` +
            `提示: 首先使用 'skill' 工具加载技能，然后调用 skill_mcp。`,
        )
      }
